...
{:else}
  ...
  <section id="quick-start">
    <h2>quick start</h2>
    <pre><code>// variables, sliders, functions
...
line l = line(slope=1, intercept=0)
segment s = segment((0,0), (3,4))
...
&#125;</code></pre>
  ...
  <section id="lexical-rules">
    <h2>lexical rules</h2>
    <ul>
      <li><strong>comments:</strong> <code>//</code> to the end of the line, or <code>/* ... */</code> across as many lines as you like.</li>
      ...
      <li><strong>keywords:</strong> <code>fn</code> <code>use</code> <code>debug</code> <code>expr</code> <code>if</code> <code>then</code> <code>else</code> <code>where</code> <code>for</code> <code>in</code> <code>as</code>. every other name in the language — <code>point</code>, <code>circle</code>, <code>time</code>, <code>step</code> and the rest — is an ordinary identifier you may shadow.</li>
      <li><strong>range tokens:</strong>
        <ul>
          <li><code>..</code> — range separator (<code>0..6.28</code>), with an optional <code>step</code></li>
          <li><code>-></code> — lambda separator (<code>t -> cos(t)</code>)</li>
        </ul>
      </li>
...
// full-line comment</code></pre>
  ...
  <section id="top-level">
    ...
    <p>there is one statement shape:</p>
    <pre><code>[type] name = expression [where condition] [as &#123; style &#125;]</code></pre>
    <p>the type in front is an optional annotation. write it and the compiler checks that the expression really builds that thing, so <code>point p = circle(...)</code> is an error. leave it out and the expression alone decides. <code>fn</code>, <code>use</code> and <code>debug</code> are the only statements that do not follow the shape.</p>
    <table>
      <thead><tr><th>form</th><th>description</th></tr></thead>
      <tbody>
        <tr><td><code>name = expr</code></td><td>variable binding</td></tr>
        <tr><td><code>name = expr where cond</code></td><td>variable restricted to where cond holds</td></tr>
        ...
        <tr><td><code>use "plugin-id"</code></td><td>asks for a plugin — no desmos output</td></tr>
        <tr><td><code>debug expr</code></td><td>compile-time only — no desmos output</td></tr>
        <tr><td><code>name = expr &#123; bindings  result &#125;</code></td><td>inline expression block; bindings substituted into result at compile time</td></tr>
        <tr><td><code>point name = (x, y)</code></td><td>named point</td></tr>
        <tr><td><code>circle name = circle(center=(h, k), radius=r)</code></td><td>circle</td></tr>
        <tr><td><code>line name = line(slope=m, intercept=b)</code></td><td>line — slope-intercept</td></tr>
        <tr><td><code>line name = lhs == rhs</code></td><td>line — standard form</td></tr>
        <tr><td><code>segment name = segment((x1,y1), (x2,y2))</code></td><td>line segment</td></tr>
        <tr><td><code>polygon name = polygon([(x,y), ...])</code></td><td>filled polygon</td></tr>
        <tr><td><code>curve name = curve(v -> body, start..end)</code></td><td>parametric curve or sampled list</td></tr>
        <tr><td><code>name = [body for v in start..end]</code></td><td>list comprehension</td></tr>
        <tr><td><code>name = map(v -> body, start..end step n)</code></td><td>the same list, written as a call</td></tr>
        <tr><td><code>region name = inequality</code></td><td>filled inequality region</td></tr>
        <tr><td><code>text name = text("label", at=(x, y))</code></td><td>text label at position</td></tr>
        <tr><td><code>group name = group("Folder label")</code></td><td>desmos folder</td></tr>
        <tr><td><code>time name = time(a..b, period=ms)</code></td><td>the clock — one auto-playing slider, see <a href="#animation">animation</a></td></tr>
        <tr><td><code>camera name = camera(azimuth=a, elevation=e)</code></td><td>the view angles <code>project()</code> reads</td></tr>
        <tr><td><code>spiral name = spiral(turns=n, spacing=s)</code></td><td>archimedean spiral</td></tr>
        <tr><td><code>wave name = wave(freq=f, amp=a)</code></td><td>sine wave</td></tr>
        <tr><td><code>grid name = grid(cols, rows)</code></td><td>cartesian grid lines</td></tr>
      </tbody>
    </table>
    <p>every call takes positional arguments first, then <code>name = value</code> keyword arguments, the way python does.</p>
  </section>
  ...
  <section id="expressions">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>list range</td><td><code>[0..1 step 0.1]</code></td></tr>
  ...
  <section id="domain">
    ...
    <p>append <code>where cond</code> to any binding to restrict where the expression is drawn:</p>
    <aside class="callout note">
      <strong>note</strong>
      <p>this filters what gets drawn. the expression itself does not change.</p>
    </aside>
    <pre><code>y = x^2 where x > 0        // only draws for x > 0
w = sin(x) where x >= -pi  // only draws for x >= -π</code></pre>
...
circle d = circle((a, b), r)              // classic form</code></pre>
    ...
    <table>
      ...
      <tbody>
        <tr><td>slope-intercept</td><td><code>line l = line(slope=2, intercept=1)</code></td><td><code>y=2x+1</code></td></tr>
...
polygon sq  = [(0,0), (2,0), (2,2), (0,2)]</code></pre>
    ...
    <h3>text</h3>
    <pre><code>text lbl = text("hello", at=(1, 2))
text note = text("f(x) = x²", at=(0, 5))</code></pre>
  ...
  <section id="curves">
    ...
    <p><code>curve</code> takes a lambda and a range, and evaluates the lambda for each sample. a tuple body produces a parametric curve; a scalar body produces a sampled list.</p>
    <pre><code>// parametric curve (tuple body)
curve ring = curve(t -> (cos(t), sin(t)), 0..6.28)

// sampled list (scalar body)
curve vals = curve(n -> n^2, 0..10)</code></pre>
    ...
    <pre><code>curve ring = curve(t -> (cos(t), sin(t)), 0..6.28 step 0.01)</code></pre>

    <h3>list comprehension</h3>
    <p>shorter than <code>curve</code>, and always a list:</p>
    <pre><code>pts = [(cos(t), sin(t)) for t in 0..6.28]</code></pre>
    <p>same meaning as <code>curve</code>, and the same compiled output.</p>

    <h3>region</h3>
    <p>any inequality or boolean expression can be a region body:</p>
    <pre><code>region r  = y > x^2
region s  = x^2 + y^2 &lt; 9
region r2 = y &lt; x as &#123; color: blue, opacity: 0.3, fill: true &#125;</code></pre>
...
// equivalent to writing: ring = (2*cos(t), sin(t))</code></pre>
    <p>use it when a long parametric body reads better with named intermediates. this is the one place braces hold statements rather than a record.</p>
...
debug r           // same — no output</code></pre>
  ...
  <section id="styling">
    <h2>styling</h2>
    <p>any geometry statement can be followed by <code>as &#123; ... &#125;</code>, a record of <code>key: value</code> pairs whose values are ordinary expressions:</p>
    <pre><code>point p2 = (0, 0) as &#123; color: red, pointSize: 12 &#125;
circle c = circle(center=(0,0), radius=3) as &#123; color: rgb(0, 128, 255), opacity: 0.2 &#125;
region r = y > x^2 as &#123; color: "#e040fb", fill: true &#125;
curve ring = curve(t -> (cos(t), sin(t)), 0..6.28) as &#123; gradient: gradient("blue", "red") &#125;
pts = [(cos(i), sin(i)) for i in 0..6.28 step 0.1] as &#123; gradient: gradient(rgb(123, 33, 22), "#33dd33") &#125;</code></pre>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>gradient(from, to)</code></td><td>two color values (named / hex / rgb / hsv)</td><td>curve, comprehension</td></tr>
...
color: hsv(240, 1, 1)   // h 0–360, s/v 0–1</code></pre>
    ...
    <h3>gradients</h3>
    <p>gradients interpolate color along the parameter or loop variable:</p>
    <pre><code>curve ring = curve(t -> (cos(t), sin(t)), 0..6.28) as &#123; gradient: gradient("blue", "red") &#125;
pts = [(cos(i), sin(i)) for i in 0..6.28 step 0.1] as &#123; gradient: gradient(rgb(123, 33, 22), "#33dd33") &#125;
curve c = curve(t -> (t, sin(t)), 0..4) as &#123; gradient: gradient("green", "orange"), opacity: 0.9 &#125;</code></pre>
...
a = slider(3, 0, 10, step=0.1, speed=1, loop=true) // step + looping auto-play</code></pre>
    ...
    <pre><code>time T = time(0..6.28, period=14000)       // one sweep every 14 seconds
time T = time(0..1)                        // period 4000, looping
time T = time(0..1, period=2000, mode=mirror) // runs forward, then back</code></pre>
    <table>
    ...
    <pre><code>time T = time(0..6.28, period=12000)

curve path = curve(t -> (2cos(t), 2sin(t)), 0..6.28)
point body = (2cos(T), 2sin(T)) as &#123; color: blue, pointSize: 12 &#125;</code></pre>
  ...
  <section id="three-d">
    ...
    <pre><code>camera cam = camera(azimuth=0.6, elevation=0.4)

time T = time(0..6.28, period=10000)
  ...
  <section id="builtins">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>gradient(from, to)</code></td><td>style property only — interpolates color along the curve or list parameter. colors can be named, hex strings, rgb(), or hsv().</td></tr>
...
// with transforms
spiral s = spiral(turns=8, spacing=0.15, cX=2, cY=1, rotate=0.5) as &#123; gradient: gradient("blue", "red") &#125;</code></pre>
    <table>
...
wave w = wave(freq=1, amp=2, xMin=-5, xMax=5, cY=3) as &#123; gradient: gradient("green", "blue") &#125;</code></pre>
    <table>
      <thead><tr><th>param</th><th>description</th><th>default</th></tr></thead>
      <tbody>
...
grid g = grid(cols=6, rows=6, xMin=-3, xMax=3, yMin=-3, yMax=3)</code></pre>
  ...
  <section id="plugins">
    ...
    <ul>
      ...
      <li><strong>libraries</strong> — <code>lib.dsmx</code> may declare only <code>fn</code> and plain bindings. the declarations are inlined at their call sites and never drawn, so a plugin cannot put anything on your graph.</li>
  ...
  <section id="errors">
    ...
    <ul>
      ...
      <li>an <code>fn</code> that nothing uses</li>
    </ul>
  </section>
  ...
  <section id="limitations">
    ...
    <ul>
      <li><strong>no bare expressions</strong> — every statement binds a name. <code>fn</code>, <code>use</code> and <code>debug</code> are the only statements that do not.</li>
      <li><strong>no negative step in ranges</strong> — <code>10..0</code> is syntactically valid but behavior is undefined.</li>
